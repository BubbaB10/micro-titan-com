...
const offeredNames = extras.filter(e => e.note !== "Not offered").map(e => e.name);
...
export default function ServiceQuote() {
  ...
  // "We'll price this scope" is a promise to quote. It must only appear when something the
  // visitor picked is scope we ACTUALLY OFFER — additional locations, or custom software.
  // Selecting only advertising or photography previously produced that sentence, which offered
  // to price two services we do not sell.
  const quotable = locations !== "one" || selected.some(name => offeredNames.includes(name));
  return <section className="quote-grid" id="quote">
    ...
    <div className="quote-summary" aria-live="polite" aria-atomic="true"><p className="eyebrow">{custom ? "YOUR PACKAGE + ADDITIONAL SCOPE" : "YOUR STANDARD PACKAGE"}</p><h3>Website + monthly care</h3><p>Single-location package</p><dl><div><dt>One-time setup</dt><dd>{dollars(offer.setup)}</dd></div><div><dt>Monthly service</dt><dd>{dollars(offer.monthly)}<small>/mo</small></dd></div></dl>
      ...
      {custom && <div className="quote-extras"><strong>Not included in the price above</strong><ul>{locations!=="one" && <li>Additional locations — quote needed</li>}{selected.map(extra=>{const meta=extras.find(item=>item.name===extra);return <li key={extra}>{extra} — {meta && meta.note==="Not offered" ? "not offered" : "quote needed"}</li>;})}</ul><p>{quotable ? "We'll price the scope we offer before you commit. The numbers above are the base package, not your complete total." : "We don't offer the items above, so there is nothing further for us to price. The numbers above are the complete package."}</p></div>}
