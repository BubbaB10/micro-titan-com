...
export default function ServiceQuote() {
  ...
  return <section className="quote-grid" id="quote">
    ...
    <div className="quote-summary" aria-live="polite" aria-atomic="true"><p className="eyebrow">{custom ? "YOUR PACKAGE + ADDITIONAL SCOPE" : "YOUR STANDARD PACKAGE"}</p><h3>Website + ongoing marketing</h3><p>Single-location package</p><dl><div><dt>One-time setup</dt><dd>{dollars(offer.setup)}</dd></div><div><dt>Monthly service</dt><dd>{dollars(offer.monthly)}<small>/mo</small></dd></div></dl>
      <p className="fine-print">Standard website, website care, Google Business Profile, local SEO, review requests, 8 social posts across 2 channels, and monthly reporting.</p>
      ...
      <a className="button" href={`mailto:hello@micro-titan.com?subject=${encodeURIComponent("My Micro Titan quote")}&body=${encodeURIComponent(`I'd like to discuss the website and marketing package: ${dollars(offer.setup)} setup + ${dollars(offer.monthly)}/month. Locations: ${locations === "one" ? "one" : "multiple"}. Additional needs: ${selected.join(", ") || "none"}.`)}`}>Discuss this quote ↗</a><p className="fine-print">Opens your email app only if you choose. Scope, terms, and start date are agreed before payment.</p>
    </div>
  </section>;
}
