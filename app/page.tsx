...
export default function HomePage() {
  return <ServicesShell>
    ...
    <section className="section wrap" id="services">
      ...
      <div className="service-columns"><article><span className="service-number">01</span><h3>Keep the work moving.</h3><p>Software for the work behind the counter: orders, reservations, estimates, jobs, and invoices. We build it around how your business already runs, then keep running it with you.</p><span className="service-label">CUSTOM APPS + OPERATIONS</span></article><article><span className="service-number">02</span><h3>An assistant comes with it.</h3><p>The software does the routine part on its own and asks a person before anything reaches a customer. You get a record of what it did and what it checked, not a promise that it worked.</p><span className="service-label">BUILT-IN ASSISTANT</span></article><article><span className="service-number">03</span><h3>Make it easy to find you.</h3><p>A clear website, kept current, with the on-page groundwork search engines look for. This is where the digital services start, and the list grows as we prove each one.</p><span className="service-label">WEBSITES + LOCAL SEARCH</span></article></div>
    </section>
...
<p className="fine-print">We will offer these when we can show one done properly. Saying so now is cheaper for both of us than saying so later.</p><Link className="button" href="/pricing#quote">Build your quote ↗</Link><p className="fine-print">Custom software, ad spend, photography, and video are separate. Scope and start date agreed before you commit.</p></div></section>
